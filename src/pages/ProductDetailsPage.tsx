    ...
    <div className="container mx-auto px-4 md:px-6 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-6">
          <div className="relative aspect-[4/5] bg-muted rounded-[2.5rem] overflow-hidden border border-border">
            <motion.img 
              key={selectedImage}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <div className="absolute top-6 left-6 flex flex-col gap-3">
               <button className="p-3 bg-black/40 backdrop-blur-md rounded-full text-primary/70 hover:text-red-400 transition-all active:scale-90">
                 <Heart size={20} />
               </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {product.images.map((img, i) => (
              <button 
                ...
                className={`aspect-square rounded-2xl overflow-hidden border-2 transition-all ${selectedImage === i ? 'border-accent ring-2 ring-accent/20' : 'border-transparent hover:border-border'}`}
              >
        ...
        <div className="flex flex-col">
          <div className="mb-8">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1 bg-yellow-900/30 text-yellow-400 px-3 py-1.5 rounded-full text-sm font-bold">
          ...
          <div className="mb-10 p-6 bg-muted rounded-[2rem] border border-border">
             <div className="flex items-end gap-3 mb-1">
               <span className="text-3xl font-bold">₹{product.price}</span>
               {product.originalPrice && (
                 <span className="text-lg text-muted-foreground line-through mb-1">₹{product.originalPrice}</span>
               )}
               <span className="text-green-400 font-bold ml-2 mb-1">40% OFF</span>
             </div>
          ...
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-2xl text-left border-2 transition-all ${lensType === type.id ? 'border-accent bg-accent/10' : 'border-border hover:border-accent/40'}`}
                  >
            ...
            {lensType !== 'frame-only' && lensType !== 'zero-power' && (
              <div className="p-6 border-2 border-dashed border-border rounded-[2rem] bg-muted text-center">
          ...
          <div className="mt-16 grid grid-cols-3 gap-4 py-8 border-y border-border">
            <div className="flex flex-col items-center text-center">
              <Truck size={24} className="mb-2 text-muted-foreground" />
              <span className="text-[10px] font-bold text-muted-foreground">FREE DELIVERY</span>
              <span className="text-[10px] text-muted-foreground/60">Estimate: 3-5 days</span>
            </div>
            <div className="flex flex-col items-center text-center">
              <RefreshCw size={24} className="mb-2 text-muted-foreground" />
              <span className="text-[10px] font-bold text-muted-foreground">14 DAYS RETURN</span>
              <span className="text-[10px] text-muted-foreground/60">Easy & Contactless</span>
            </div>
            <div className="flex flex-col items-center text-center">
              <ShieldCheck size={24} className="mb-2 text-muted-foreground" />
              <span className="text-[10px] font-bold text-muted-foreground">WARRANTY</span>
              <span className="text-[10px] text-muted-foreground/60">1 Year Frame Protection</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-24">
        <div className="border-b border-border flex gap-12 mb-12 overflow-x-auto no-scrollbar">
          <button className="pb-4 border-b-2 border-accent font-bold text-accent">Specifications</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          ...
          <div className="bg-muted p-8 rounded-[2.5rem]">
