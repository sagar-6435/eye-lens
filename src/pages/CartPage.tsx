    ...
    <div className="container mx-auto px-4 md:px-6 py-12">
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <div className="flex-[2]">
            ...
            <AnimatePresence mode="popLayout">
              {items.map((item) => (
                <motion.div 
                  ...
                  className="flex gap-6 p-6 rounded-[2rem] border border-border group hover:border-accent/40 transition-all bg-muted"
                >
                  <Link to={`/product/${item.id}`} className="w-32 sm:w-40 aspect-square bg-secondary rounded-2xl overflow-hidden shrink-0 border border-border p-2">
                    ...
                    <div className="flex items-end justify-between">
                      <div className="flex items-center gap-3 bg-secondary p-1.5 rounded-xl">
                        <button 
                          ...
                          className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center hover:bg-accent hover:text-secondary transition-all active:scale-90"
                        >
                          <Minus size={14} />
                        </button>
          ...
          <div className="mt-12 p-8 bg-muted rounded-[2.5rem] border border-dashed border-border">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               {[
                 ...
                 <div key={i} className="bg-secondary p-4 rounded-2xl flex items-center justify-between border border-border group hover:border-accent transition-all">
                    <div>
                      <p className="font-bold text-sm mb-1">{offer.title}</p>
                      <p className="text-xs text-muted-foreground">{offer.desc}</p>
                    </div>
                    <div className="px-3 py-1 bg-muted border border-dashed border-border rounded text-[10px] font-bold group-hover:bg-accent/10 group-hover:border-accent transition-all">
                      {offer.code}
                    </div>
                 </div>
               ))}
        ...
        <div className="flex-1">
          <div className="sticky top-32 space-y-8">
            <div className="bg-muted p-8 rounded-[2.5rem] border border-border shadow-premium">
               ...
               <div className="space-y-4 mb-8">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={`font-bold ${shipping === 0 ? 'text-green-400' : ''}`}>
               ...
               <div className="flex flex-col gap-4">
                 <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl">
                   ...
                   <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Secure Payment & Buyer Protection</p>
                 </div>
               </div>
            </div>

            <div className="p-6 bg-muted rounded-[2rem] border border-border flex items-center gap-4">
